## 🔴 The Problem

When a user downloads a large file, the server needs 10-120 seconds to process it. But proxies like Cloudflare have a 100-second timeout. If processing takes longer, the connection dies.

```
                    ❌ THE PROBLEM: Synchronous Download
    ┌─────────────────────────────────────────────────────────────────────────┐
    │                                                                         │
    │   User clicks "Download"                                                │
    │          │                                                              │
    │          ▼                                                              │
    │   ┌──────────┐      ┌─────────────┐      ┌─────────────┐                │
    │   │  Browser │ ───► │ Cloudflare  │ ───► │ API Server  │                │
    │   └──────────┘      │ (100s limit)│      └─────────────┘                │
    │          │          └─────────────┘             │                       │
    │          │                │                     │                       │
    │     Waiting...      TIMEOUT!           Still working...                 │
    │          │                │                     │                       │
    │          ▼                ▼                     ▼                       │
    │   ┌──────────┐      ┌─────────────┐      ┌─────────────┐                │
    │   │  Error!  │ ◄─── │ Connection  │      │   Done!     │                │
    │   │   504    │      │    Cut      │      │ (too late)  │                │
    │   └──────────┘      └─────────────┘      └─────────────┘                │
    │                                                                         │
    │   Result: User gets error, server wasted work, file never delivered     │
    └─────────────────────────────────────────────────────────────────────────┘
```

### The Timeout Reality

| Layer      | Timeout |     Can We Change It?     |
| :--------- | :-----: | :-----------------------: |
| Cloudflare |  100s   | ❌ No (unless Enterprise) |
| AWS ALB    |   60s   |        ⚠️ Limited         |
| nginx      |   60s   |          ✅ Yes           |
| Browser    | Varies  |           ❌ No           |

**Bottom line:** We can't control proxy timeouts, so we must work around them.

---

## ✅ Our Solution

Instead of making the user wait, we:

1. **Return a job ID instantly** (< 100ms)
2. **Process in the background**
3. **Push updates via SSE** (with polling fallback)

```
                           ✅ OUR SOLUTION: Async Download
    ┌─────────────────────────────────────────────────────────────────────────┐
    │                                                                         │
    │   User clicks "Download"                                                │
    │          │                                                              │
    │          ▼                                                              │
    │   ┌──────────┐      ┌─────────────┐      ┌─────────────┐               │
    │   │  Browser │ ───► │ Cloudflare  │ ───► │ API Server  │               │
    │   └──────────┘      └─────────────┘      └─────────────┘               │
    │          │                                      │                       │
    │          │◄──── { job_id: "abc-123" } ◄────────┘                       │
    │          │         (< 100ms!)                                           │
    │          │                                                              │
    │          ▼                                      ▼                       │
    │   ┌──────────┐                           ┌─────────────┐               │
    │   │ Subscribe│                           │   Worker    │               │
    │   │ to SSE   │                           │ (background)│               │
    │   └──────────┘                           └─────────────┘               │
    │          │                                      │                       │
    │          │◄──── Progress: 33% ◄─────────────────┤                       │
    │          │◄──── Progress: 67% ◄─────────────────┤                       │
    │          │◄──── Progress: 100% ◄────────────────┤                       │
    │          │◄──── Completed! {url} ◄──────────────┘                       │
    │          │                                                              │
    │          ▼                                                              │
    │   ┌──────────┐      ┌─────────────┐                                    │
    │   │ Download │ ───► │  MinIO S3   │                                    │
    │   │  File    │      │  (storage)  │                                    │
    │   └──────────┘      └─────────────┘                                    │
    │                                                                         │
    │   Result: User gets file, no timeout, real-time progress ✅            │
    └─────────────────────────────────────────────────────────────────────────┘
```

---

## 🏛️ Architecture Diagram

### System Overview

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                              SYSTEM ARCHITECTURE                                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│    ┌─────────────────┐                                                          │
│    │   👤 CLIENT     │                                                          │
│    │  React Frontend │                                                          │
│    │     :5173       │                                                          │
│    └────────┬────────┘                                                          │
│             │                                                                    │
│             │ HTTPS                                                              │
│             ▼                                                                    │
│    ┌─────────────────┐                                                          │
│    │  🌐 PROXY       │                                                          │
│    │   Cloudflare    │◄─── 100s timeout (we respond in <100ms)                  │
│    │   or nginx      │                                                          │
│    └────────┬────────┘                                                          │
│             │                                                                    │
│             ▼                                                                    │
│    ┌─────────────────┐         ┌─────────────────┐                              │
│    │   ⚡ API        │         │ 📊 OBSERVABILITY│                              │
│    │  Hono Server    │────────►│  Jaeger :16686  │                              │
│    │     :3000       │         │  Sentry         │                              │
│    └────────┬────────┘         └─────────────────┘                              │
│             │                                                                    │
│      ┌──────┴──────┐                                                            │
│      │             │                                                            │
│      ▼             ▼                                                            │
│ ┌─────────┐  ┌───────────┐                                                      │
│ │📋 QUEUE │  │ 📁 JOB   │                                                      │
│ │SimpleQ  │  │  STORE    │───────┐                                              │
│ │FIFO     │  │ In-Memory │       │ SSE Events                                   │
│ │Conc: 5  │  │ EventEmit │       │                                              │
│ └────┬────┘  └───────────┘       │                                              │
│      │                           │                                              │
│      ▼                           ▼                                              │
│ ┌─────────┐              ┌─────────────────┐                                    │
│ │⚙️ WORKER│              │   👤 CLIENT     │                                    │
│ │Background│             │ (receives SSE)  │                                    │
│ │Processor │              └─────────────────┘                                   │
│ └────┬────┘                                                                     │
│      │                                                                          │
│      ▼                                                                          │
│ ┌─────────────────┐                                                             │
│ │  💾 STORAGE     │                                                             │
│ │   MinIO S3      │                                                             │
│ │  :9000 / :9001  │                                                             │
│ │                 │                                                             │
│ │ • Store files   │                                                             │
│ │ • Presigned URLs│                                                             │
│ │ • 1hr expiry    │                                                             │
│ └─────────────────┘                                                             │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### Request Flow (Sequence)

```
┌────────┐          ┌─────────┐          ┌───────┐          ┌────────┐          ┌───────┐
│ Client │          │   API   │          │ Queue │          │ Worker │          │  S3   │
└───┬────┘          └────┬────┘          └───┬───┘          └───┬────┘          └───┬───┘
    │                    │                   │                  │                   │
    │ 1. POST /async     │                   │                  │                   │
    │ ─────────────────► │                   │                  │                   │
    │                    │ 2. Enqueue job    │                  │                   │
    │                    │ ────────────────► │                  │                   │
    │                    │                   │                  │                   │
    │ 3. {job_id}        │                   │                  │                   │
    │ ◄───────────────── │                   │                  │                   │
    │    (<100ms!)       │                   │                  │                   │
    │                    │                   │                  │                   │
    │ 4. Subscribe SSE   │                   │                  │                   │
    │ ─────────────────► │                   │                  │                   │
    │                    │                   │ 5. Dequeue       │                   │
    │                    │                   │ ───────────────► │                   │
    │                    │                   │                  │                   │
    │                    │                   │                  │ 6. Process files  │
    │                    │                   │                  │ ─────────────────►│
    │ 7. SSE: 33%        │                   │                  │                   │
    │ ◄══════════════════╪═══════════════════╪══════════════════╡                   │
    │ 8. SSE: 67%        │                   │                  │                   │
    │ ◄══════════════════╪═══════════════════╪══════════════════╡                   │
    │ 9. SSE: 100%       │                   │                  │                   │
    │ ◄══════════════════╪═══════════════════╪══════════════════╡                   │
    │                    │                   │                  │                   │
    │                    │                   │                  │ 10. Upload        │
    │                    │                   │                  │ ────────────────► │
    │                    │                   │                  │                   │
    │                    │                   │                  │ 11. Presigned URL │
    │                    │                   │                  │ ◄──────────────── │
    │                    │                   │                  │                   │
    │ 12. SSE: {url}     │                   │                  │                   │
    │ ◄══════════════════╪═══════════════════╪══════════════════╡                   │
    │                    │                   │                  │                   │
    │ 13. Download file  │                   │                  │                   │
    │ ──────────────────────────────────────────────────────────────────────────► │
    │                    │                   │                  │                   │
┌───┴────┐          ┌────┴────┐          ┌───┴───┐          ┌───┴────┐          ┌───┴───┐
│ Client │          │   API   │          │ Queue │          │ Worker │          │  S3   │
└────────┘          └─────────┘          └───────┘          └────────┘          └───────┘
```

### Job State Machine

```
                              JOB LIFECYCLE
    ┌─────────────────────────────────────────────────────────────────┐
    │                                                                 │
    │                        ┌──────────┐                             │
    │        POST /async ──► │  QUEUED  │                             │
    │                        └─────┬────┘                             │
    │                              │                                  │
    │                              │ Worker picks up                  │
    │                              ▼                                  │
    │                        ┌──────────────┐                         │
    │                        │  PROCESSING  │◄─────┐                  │
    │                        └───────┬──────┘      │                  │
    │                                │             │                  │
    │                    ┌───────────┼───────────┐ │                  │
    │                    │           │           │ │ Retry            │
    │                    ▼           ▼           ▼ │ (max 3)          │
    │              ┌─────────┐ ┌─────────┐ ┌─────────┐                │
    │              │  READY  │ │  ERROR  │─┘         │                │
    │              │   ✅    │ │   ⚠️    │           │                │
    │              └─────────┘ └────┬────┘           │                │
    │                               │                │                │
    │                               │ Max retries    │                │
    │                               ▼                │                │
    │                         ┌──────────┐           │                │
    │                         │  FAILED  │           │                │
    │                         │    ❌    │           │                │
    │                         └──────────┘           │                │
    │                                                                 │
    └─────────────────────────────────────────────────────────────────┘
```

---

## 🤔 Why This Approach?

### The Four Options We Considered

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                         PATTERN COMPARISON                                       │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│   OPTION A: POLLING ONLY              OPTION B: SSE ONLY                        │
│   ──────────────────────              ──────────────────                        │
│                                                                                  │
│   Client        Server                Client        Server                      │
│      │            │                      │            │                         │
│      │──Request──►│                      │──Subscribe─►│                         │
│      │◄─Response──│                      │             │                         │
│      │   wait 2s  │                      │◄──Event 1───│                         │
│      │──Request──►│                      │◄──Event 2───│                         │
│      │◄─Response──│                      │◄──Event 3───│                         │
│      │   wait 2s  │                      │◄──Done──────│                         │
│      │──Request──►│                      │             │                         │
│      │   ...      │                                                              │
│                                                                                  │
│   ❌ Wastes bandwidth                 ❌ May not work behind                    │
│   ❌ Delayed updates                     strict proxies                         │
│   ✅ Works everywhere                 ✅ Real-time                               │
│                                                                                  │
│ ─────────────────────────────────────────────────────────────────────────────── │
│                                                                                  │
│   OPTION C: WEBHOOK                   OPTION D: HYBRID (OURS) ✅                │
│   ─────────────────                   ──────────────────────────                │
│                                                                                  │
│   Client        Server                Client        Server                      │
│      │            │                      │            │                         │
│      │──Request──►│                      │──Request──►│                         │
│      │ {callback} │                      │◄──job_id───│  (<100ms)               │
│      │            │                      │            │                         │
│      │   ...      │                      │──Try SSE──►│                         │
│      │            │                      │◄──Events───│  (if works)             │
│      │◄─Callback──│                      │            │                         │
│                                          │──OR Poll──►│  (fallback)             │
│   ❌ Client needs server              │◄──Status───│                         │
│   ❌ Complex setup                       │            │                         │
│   ✅ Efficient                                                                   │
│                                       ✅ Real-time when possible                │
│                                       ✅ Works everywhere                        │
│                                       ✅ Graceful degradation                    │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### Decision Matrix

| Requirement             | Polling | SSE Only | Webhook | **Hybrid** ✅ |
| :---------------------- | :-----: | :------: | :-----: | :-----------: |
| Works behind Cloudflare |   ✅    |    ⚠️    |   ✅    |      ✅       |
| Real-time updates       |   ❌    |    ✅    |   ✅    |      ✅       |
| Browser-only client     |   ✅    |    ✅    |   ❌    |      ✅       |
| Simple implementation   |   ✅    |    ✅    |   ❌    |      ✅       |
| No wasted requests      |   ❌    |    ✅    |   ✅    |      ✅       |
| Graceful degradation    |   N/A   |    ❌    |   ❌    |      ✅       |

**Winner: Hybrid** — Gets real-time updates when possible, falls back to polling when needed.

---

## ❓ What If We Used Other Approaches?

### ❌ Synchronous (No Queue)

```
What happens:
├── User clicks download
├── Server starts processing
├── 60 seconds pass...
├── Cloudflare: "Timeout!" 💀
├── User sees 504 error
├── User refreshes → creates DUPLICATE job
├── Server overloaded
└── User never gets file
```

### ❌ Polling Only

```
What happens:
├── 30 requests for a 60-second job
├── 2-second delay before seeing "done"
├── Server handles unnecessary load
├── Mobile users waste data
└── Works, but inefficient
```

### ❌ SSE Only (No Fallback)

```
What happens:
├── Corporate proxy blocks SSE
├── User gets no updates
├── No way to know if it's working
├── User refreshes (duplicate job)
└── Poor experience for some users
```

### ✅ Our Hybrid

```
What happens:
├── Try SSE first
├── If SSE works → real-time updates
├── If SSE fails → automatic polling fallback
├── All users get updates
└── Everyone gets their file ✅
```

---

## ⚖️ Pros and Cons

### ✅ Advantages

| Benefit                   | Explanation                              |
| :------------------------ | :--------------------------------------- |
| **🚀 No Timeouts**        | Response in <100ms beats any proxy limit |
| **📊 Real-time Progress** | Users see 33% → 67% → 100% live          |
| **🔄 Resilient**          | Server restart doesn't kill downloads    |
| **📈 Scalable**           | Add workers to handle more jobs          |
| **🛡️ Universal**          | Works for all users (SSE or polling)     |
| **🔍 Observable**         | Full tracing with Jaeger                 |

### ❌ Limitations

| Limitation           | Our Mitigation             |
| :------------------- | :------------------------- |
| **In-memory queue**  | Use Redis in production    |
| **More complex**     | Worth it for reliability   |
| **SSE proxy issues** | Automatic polling fallback |
| **No auth**          | Add JWT in production      |

---

## 🔧 Implementation Details

### API Endpoints

| Method | Endpoint                     | Purpose      | Response                   |
| :----- | :--------------------------- | :----------- | :------------------------- |
| `POST` | `/v1/download/async`         | Start job    | `{job_id}` in <100ms       |
| `GET`  | `/v1/download/status/:id`    | Poll status  | `{status, progress, url?}` |
| `GET`  | `/v1/download/subscribe/:id` | SSE stream   | Real-time events           |
| `GET`  | `/health`                    | Health check | `{api, storage}`           |

### Data Schemas

```typescript
// Request
interface DownloadRequest {
  file_ids: number[]; // [101, 102, 103]
}

// Job Status Response
interface JobStatus {
  job_id: string; // "abc-123"
  status: "queued" | "processing" | "ready" | "failed";
  progress: number; // 0-100
  total_files: number; // 3
  processed_files: number; // 2
  download_url?: string; // When ready
  error?: string; // When failed
}

// SSE Events
type SSEEvent =
  | { type: "progress"; data: { progress: number } }
  | { type: "completed"; data: { download_url: string } }
  | { type: "failed"; data: { error: string } };
```

### Queue Configuration

| Setting     | Value        | Purpose                           |
| :---------- | :----------- | :-------------------------------- |
| Type        | FIFO         | First-in-first-out processing     |
| Concurrency | 5            | Process 5 jobs simultaneously     |
| Retries     | 3            | Retry failed jobs 3 times         |
| Backoff     | 1s → 2s → 4s | Exponential delay between retries |

### Error Handling

```
                    ERROR HANDLING FLOW
    ┌─────────────────────────────────────────────────┐
    │                                                 │
    │   Job received                                  │
    │        │                                        │
    │        ▼                                        │
    │   ┌─────────┐                                   │
    │   │ Process │                                   │
    │   │  file   │                                   │
    │   └────┬────┘                                   │
    │        │                                        │
    │   ┌────┴────┐                                   │
    │   │         │                                   │
    │   ▼         ▼                                   │
    │ Success?  Failed?                               │
    │   │         │                                   │
    │   │         ├──► Retry < 3? ──► Wait & retry   │
    │   │         │                                   │
    │   │         └──► Retry = 3? ──► Mark FAILED    │
    │   │                                             │
    │   └──► More files? ──► Process next            │
    │              │                                  │
    │              └──► Done ──► Upload to S3        │
    │                              │                  │
    │                              └──► Mark READY   │
    │                                                 │
    └─────────────────────────────────────────────────┘
```

---

## 🌐 Proxy Configuration

### Timeout Strategy

```
    ┌─────────────────────────────────────────────────────────────────┐
    │                     TIMEOUT CONFIGURATION                        │
    ├─────────────────────────────────────────────────────────────────┤
    │                                                                  │
    │   Layer            Timeout      Our Response Time                │
    │   ─────────────────────────────────────────────                 │
    │   Cloudflare       100s         <100ms ✅                       │
    │   nginx            60s          <100ms ✅                       │
    │   API Request      30s          <100ms ✅                       │
    │   SSE Connection   ∞            Streaming ✅                    │
    │   Presigned URL    1 hour       N/A                             │
    │                                                                  │
    │   ✅ We respond BEFORE any timeout can trigger                  │
    │                                                                  │
    └─────────────────────────────────────────────────────────────────┘
```

### nginx Configuration

```nginx
# SSE endpoints - no buffering, long timeout
location /v1/download/subscribe/ {
    proxy_pass http://api:3000;
    proxy_buffering off;           # Don't buffer SSE
    proxy_cache off;               # Don't cache
    proxy_read_timeout 3600s;      # 1 hour for long jobs
    proxy_set_header Connection "";
}

# Regular API - standard timeout
location /v1/download/ {
    proxy_pass http://api:3000;
    proxy_read_timeout 30s;        # Quick (we respond fast)
}
```

### Cloudflare Settings

| Setting    | Value        | Why                       |
| :--------- | :----------- | :------------------------ |
| WebSockets | ✅ On        | Required for SSE          |
| Cache      | Bypass       | Don't cache API responses |
| Timeout    | 100s (fixed) | We work around it         |

---

## 🚀 Future Improvements

| Priority  | Improvement   | Benefit               |
| :-------- | :------------ | :-------------------- |
| 🔴 High   | Redis Queue   | Jobs survive restarts |
| 🔴 High   | JWT Auth      | Secure API access     |
| 🟡 Medium | Rate Limiting | Prevent abuse         |
| 🟡 Medium | Cancel API    | Users can abort jobs  |
| 🟢 Low    | WebSocket     | Bi-directional comms  |
| 🟢 Low    | Kubernetes    | Auto-scaling          |

### Scaling Path

```
    CURRENT (Hackathon)                    PRODUCTION (Future)
    ───────────────────                    ────────────────────

    ┌─────┐   ┌───────┐   ┌────────┐      ┌──────────────────────┐
    │ API │──►│ Queue │──►│ Worker │      │    Load Balancer     │
    │ (1) │   │(memory)│  │  (1)   │      └──────────┬───────────┘
    └─────┘   └───────┘   └────────┘                 │
         │                     │           ┌─────────┼─────────┐
         └─────────────────────┘           ▼         ▼         ▼
              MinIO (1)                ┌─────┐   ┌─────┐   ┌─────┐
                                       │API 1│   │API 2│   │API N│
    Throughput: ~5 jobs                └──┬──┘   └──┬──┘   └──┬──┘
                                          └────────┼────────┘
                                                   ▼
                                          ┌──────────────┐
                                          │    Redis     │
                                          │   (BullMQ)   │
                                          └──────┬───────┘
                                          ┌──────┼──────┐
                                          ▼      ▼      ▼
                                       ┌────┐ ┌────┐ ┌────┐
                                       │ W1 │ │ W2 │ │ WN │
                                       └────┘ └────┘ └────┘
                                          │      │      │
                                          └──────┼──────┘
                                                 ▼
                                          ┌──────────────┐
                                          │  S3 Cluster  │
                                          └──────────────┘

                                       Throughput: N × 5 jobs
```

---

## 📊 Summary

| Question                  | Answer                                    |
| :------------------------ | :---------------------------------------- |
| **What problem?**         | Downloads timeout after 60-100s           |
| **Your solution?**        | Async processing, return job ID instantly |
| **Why hybrid?**           | SSE for speed + polling for compatibility |
| **Why not polling only?** | Wastes bandwidth, delayed updates         |
| **Why not SSE only?**     | Some proxies block it                     |
| **Main pros?**            | No timeouts, real-time, scalable          |
| **Main cons?**            | In-memory state (use Redis in prod)       |
| **Future?**               | Redis, auth, rate limiting                |

---

    CURRENT (Hackathon)                    PRODUCTION (Future)
    ───────────────────                    ────────────────────

    ┌─────┐   ┌───────┐   ┌────────┐      ┌──────────────────────┐
    │ API │──►│ Redis │──►│ Worker │      │    Load Balancer     │
    │ (1) │   │ Queue │   │  (1)   │      └──────────┬───────────┘
    └─────┘   └───────┘   └────────┘                 │
         │                     │           ┌─────────┼─────────┐
         └─────────────────────┘           ▼         ▼         ▼
              MinIO (1)                ┌─────┐   ┌─────┐   ┌─────┐
                                       │API 1│   │API 2│   │API N│
    Throughput: ~5 jobs                └──┬──┘   └──┬──┘   └──┬──┘
                                          └────────┼────────┘
                                                   ▼
                                          ┌──────────────┐
                                          │  Redis (Q)   │
                                          └──────┬───────┘
                                          ┌──────┼──────┐
                                          ▼      ▼      ▼
                                       ┌────┐ ┌────┐ ┌────┐
                                       │ W1 │ │ W2 │ │ WN │
                                       └────┘ └────┘ └────┘
                                          │      │      │
                                          └──────┼──────┘
                                                 ▼
                                          ┌──────────────┐
                                          │  S3 Cluster  │
                                          └──────────────┘

                                       Throughput: N × 5 jobs
